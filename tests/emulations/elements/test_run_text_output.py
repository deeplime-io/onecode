...
def test_execute():
    ...
    with open(os.path.join(flow_dir, 'flows', f'{flow_id}.py'), 'a') as f:
        f.write("""
    x = onecode.text_output('My Text Output', 'test_file.txt', label="My Label", tags=['Core'])
...
def test_invalid_name_key():
    ...
    with open(os.path.join(flow_dir, 'flows', f'{flow_id}.py'), 'a') as f:
        f.write("""
    try:
        x = onecode.text_output('_my_text_output', 'test_file.txt', tags=['Core'])
    ...
    with open(os.path.join(flow_dir, 'stdout.txt')) as f:
        assert f.read() == 'Key starting with "_" are reserved: _my_text_output'
