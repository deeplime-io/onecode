...
def test_execute():
    ...
    with open(os.path.join(flow_dir, 'flows', f'{flow_id}.py'), 'a') as f:
        f.write("""
    x = onecode.pyvista_vrml_output('My Pyvista VRML Output', 'test_file.vrml', label=None, tags=['VRML', 'Core'])
    ...
    with open(os.path.join(flow_dir, 'stdout.txt')) as f:
        assert f.read() == f"{flow_data}/outputs/test_file.vrml"

    with open(os.path.join(flow_data, "outputs", flow_id, "MANIFEST.txt")) as f:
        assert json.loads(f.read()) == {
            "key": "my_pyvista_vrml_output",
            "label": "My Pyvista VRML Output",
            "value": f"{flow_data}/outputs/test_file.vrml",
            "tags": ["VRML", "Core"],
...
def test_invalid_execute():
    ...
    with open(os.path.join(flow_dir, 'flows', f'{flow_id}.py'), 'a') as f:
        f.write("""
    try:
        x = onecode.pyvista_vrml_output('my_pyvista_vrml_output', 'test_file.txt', tags=['VRML', 'Core'])
    ...
    with open(os.path.join(flow_dir, 'stdout.txt')) as f:
        assert f.read() == strip("""
            [my_pyvista_vrml_output] Invalid VRML extension:
            .txt (accepted: .vrml)
        """)
...
def test_invalid_name_key():
    ...
    with open(os.path.join(flow_dir, 'flows', f'{flow_id}.py'), 'a') as f:
        f.write("""
    try:
        x = onecode.pyvista_vrml_output('_my_pyvista_vrml_output', 'test_file.vrml', tags=['VRML', 'Core'])
    except ValueError as err:
        with open("stdout.txt", 'w') as f:
            f.write(str(err))
    """)
    ...
    with open(os.path.join(flow_dir, 'stdout.txt')) as f:
        assert f.read() == 'Key starting with "_" are reserved: _my_pyvista_vrml_output'
