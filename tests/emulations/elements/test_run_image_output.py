...
def test_execute():
    ...
    with open(os.path.join(flow_dir, 'flows', f'{flow_id}.py'), 'a') as f:
        f.write("""
    x = onecode.image_output('My Image Output', 'test_file.jpg', tags=['Image', 'Core'])
    ...
    with open(os.path.join(flow_dir, 'stdout.txt')) as f:
        assert f.read() == f"{flow_data}/outputs/test_file.jpg"

    with open(os.path.join(flow_data, "outputs", flow_id, "MANIFEST.txt")) as f:
        assert json.loads(f.read()) == {
            "key": "my_image_output",
            "label": "My Image Output",
            "value": f"{flow_data}/outputs/test_file.jpg",
            "tags": ["Image", "Core"],
            "kind": "ImageOutput"
        }
...
def test_invalid_execute():
    ...
    with open(os.path.join(flow_dir, 'flows', f'{flow_id}.py'), 'a') as f:
        f.write("""
    try:
        x = onecode.image_output('my_image_output', 'test_file.jxpg', tags=['Image', 'Core'])
    ...
    with open(os.path.join(flow_dir, 'stdout.txt')) as f:
        assert f.read() == strip("""
            [my_image_output] Invalid image extension:
            .jxpg (accepted: .jpg, .jpeg, .png, .svg)
        """)
...
def test_invalid_name_key():
    ...
    with open(os.path.join(flow_dir, 'flows', f'{flow_id}.py'), 'a') as f:
        f.write("""
    try:
        x = onecode.image_output('_my_image_output', 'test_file.jpg', tags=['Image', 'Core'])
    except ValueError as err:
        with open("stdout.txt", 'w') as f:
            f.write(str(err))
    """)
    ...
    with open(os.path.join(flow_dir, 'stdout.txt')) as f:
        assert f.read() == 'Key starting with "_" are reserved: _my_image_output'
